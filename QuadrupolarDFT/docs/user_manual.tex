...
\section{Convergence of $\eta$ and the asymmetry gap}
...
Convergence --- and, because the inputs run with \code{nsym 1}, symmetry pinning
...
\paragraph{All-electron cross-check (Elk): the gap is a PAW artifact.} The
...
the long-standing $\eta$ under-prediction is a \textbf{PAW/pseudopotential
artifact} --- the Pseudodojo PBE nitrogen dataset under-represents the
near-nucleus transverse EFG --- \emph{not} a functional or dynamics deficiency.
...
fix is a harder / EFG-tuned nitrogen PAW dataset; a meta-GGA or hybrid functional
is not needed. The Elk cross-check workflow --- input, an \code{EFG.OUT} parser,
and an MPI-parallel runner with a convergence watchdog --- is in
\code{examples/elk/} (see its \code{README.md}); the original ranked plan is in
\code{docs/eta\_accuracy\_improvement.md}.
...
\textbf{Interpretation.} The NaNO$_2$ asymmetry error is therefore not fixed by
choosing a different bundled CIF. The dominant missing ingredient is more likely
the local equilibrium tensor itself: convergence of the EFG tensor components,
cell/internal-coordinate relaxation against the chosen functional and PAW data,
possible functional or PAW-dataset dependence, and eventually anharmonic or
snapshot averaging close to the ferroelectric transition. High-temperature
\code{I m m m} CIFs should also be treated carefully: the available entries are
