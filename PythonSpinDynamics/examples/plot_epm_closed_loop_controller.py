"""Closed-loop EPM imaging and magnetic-aggregate delivery controller."""
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Alternate nonlinear imaging, target localization, EPM state "
            "programming, and magnetophoretic transport bursts."
        )
    )
...
def main() -> None:
    ...
    fig.suptitle("Closed-loop EPM image-localize-program-transport controller", fontsize=15)
    ...
    for cycle, color in zip(result.cycles, colors, strict=True):
        ...
        ax.arrow(start[0], start[1], delta[0], delta[1], color=color, width=0.10, head_width=0.75, length_includes_head=True)
        ax.plot(start[0], start[1], "o", color=color, ms=5)
    ax.add_patch(Circle(1e3 * last_cycle.target_center_m, 1e3 * config.target_radius_m, fill=False, color="tab:red", lw=2))
    ax.set(xlim=extent[:2], ylim=extent[2:], xlabel="x (mm)", ylabel="y (mm)", title="feedback decision: uncaptured centroid to target")
    ax.grid(alpha=0.2)
    ...
    ax.add_patch(Circle(1e3 * last_cycle.target_center_m, 1e3 * config.target_radius_m, fill=False, color="tab:red", lw=2))
    ax.set(xlim=extent[:2], ylim=extent[2:], xlabel="x (mm)", ylabel="y (mm)", title=f"cycle-colored trajectories; {100 * result.capture_fraction:.1f}% captured")
    ax.grid(alpha=0.2)
    ...
    capture_axis.step(cycle_end, 100 * result.capture_fraction_by_cycle, where="post", color="tab:red", lw=2)
    capture_axis.set_ylabel("capture (%)", color="tab:red")
    capture_axis.set_ylim(0.0, 100.0)

    ax = axes[1, 2]
    indices = np.arange(1, len(result.cycles) + 1)
    gains = 100 * np.asarray([cycle.newly_captured_fraction for cycle in result.cycles])
    program_variation = np.asarray([cycle.transport_remanence_variation_t for cycle in result.cycles])
    bars = ax.bar(indices - 0.18, gains, width=0.36, color="tab:red", label="new capture (%)")
    ax.set(xlabel="controller cycle", ylabel="new capture (%)", title="cycle outcome and transport-state change")
    program_axis = ax.twinx()
    program_axis.bar(indices + 0.18, program_variation, width=0.36, color="tab:orange", label="transport state L1 change")
    program_axis.set_ylabel("summed |delta Br| (channel T)", color="tab:orange")
    handles = [bars, program_axis.containers[0]]
    ax.legend(handles, ["new capture", "transport-state change"], fontsize=8)
    ...
    print("Closed-loop EPM therapy controller")
    ...
    print(f"  capture by cycle={100 * result.capture_fraction_by_cycle} percent")
    print(f"  final capture={100 * result.capture_fraction:.1f}%")
    ...
    print("  next: estimate particle distribution from imaging and add tissue/vascular transport")
