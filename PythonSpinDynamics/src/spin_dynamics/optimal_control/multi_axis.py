...
def build_model(config: MultiAxisSLSEConfig) -> dict:
...
def control_bounds(config: MultiAxisSLSEConfig, *, amp_max: float = 4.0) -> list:
...
if JAX_AVAILABLE:
    ...
    def _segment_multi(h_drift, hx_stack, hy_stack, amp_vec, phase_vec, dt):
        ...
        return _jax_expm(-1j * (h_drift + drive) * dt)

    def _composite_unitary(h_drift, hx_stack, hy_stack, amplitude, phase, dt):
        """Compose one multi-coil pulse (n_fine segments) into a unitary."""

        def body(u, xs):
            amp_j, phase_j = xs
            return _segment_multi(h_drift, hx_stack, hy_stack, amp_j, phase_j, dt) @ u, None

        u0 = jnp.eye(h_drift.shape[0], dtype=jnp.complex128)
        u_final, _ = jax.lax.scan(body, u0, (amplitude, phase))
        return u_final
    ...
    def _make_energy_fn(config: MultiAxisSLSEConfig):
        ...
        def energy(x):
            ...
            def u_excite(hxs, hys):
                return _composite_unitary(h_drift, hxs, hys, amp_ex, ph_ex, dt_ex)

            def u_refocus(hxs, hys):
                return _composite_unitary(h_drift, hxs, hys, amp_re, ph_re, dt_re)

            u_ex = jax.vmap(u_excite)(hx_batch, hy_batch)
            u_re = jax.vmap(u_refocus)(hx_batch, hy_batch)
...
def slse_train_amplitudes(config: MultiAxisSLSEConfig, x: np.ndarray) -> np.ndarray:
    ...
    def render(pulse, window_T):
        ...
        rise = 1.0 / (1.0 + np.exp(-(centers[:, None] - start[None, :]) / width))
        fall = 1.0 / (1.0 + np.exp(-(end[None, :] - centers[:, None]) / width))
