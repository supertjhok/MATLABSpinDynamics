...
| Susceptibility-aware EPM particle spin echo | A selected retained EPM state supplies the background B0 map; field-dependent equivalent-sphere particle moments generate subvoxel dipole perturbations while magnetic-core samples exclude water signal; paired finite-pulse spin-warp acquisitions propagate tissue relaxation and diffusing water; and the reconstructed magnitude difference yields image-derived susceptibility foci and centroids without reading particle truth. | **A**, **R** | partially validated |
...
| Alternating image-guided EPM therapy controller | The controller executes contiguous image-program-transport-verification intervals, supports both the calibrated direct-signal baseline and susceptibility-aware paired spin echoes, re-aims each affine transport objective from the reconstructed outside-target centroid, and stops reproducibly from verification imaging rather than simulator truth. | **A**, **R** | partially validated |
...
- **Claim:** A selected retained EPM state supplies the background B0 map; field-dependent equivalent-sphere particle moments generate subvoxel dipole perturbations while magnetic-core samples exclude water signal; paired finite-pulse spin-warp acquisitions propagate tissue relaxation and diffusing water; and the reconstructed magnitude difference yields image-derived susceptibility foci and centroids without reading particle truth.
- **Evidence:** A, R (partially validated)
- **Basis:** Uniformly magnetized sphere interior and exterior dipole identities, direct EPM field-basis evaluation, explicit 90--180 spin-warp propagation, paired seeded random-walker diffusion, and synthetic contrast-localization fixtures.
- **Tested range:** Two-dimensional uniform Cartesian tissue maps; one through nine subvoxel samples per axis; non-negative water diffusion; one or more walkers per voxel; linear or Langevin particle magnetization; finite excitation, refocusing, phase-encoding, and readout durations; optional complex k-space noise.
- **Metric:** Dipole-field sign and symmetry, nonzero particle delta B0, selected EPM background field, spatial subvoxel propagation, echo time, resolved contrast foci, centroid error, controller consumption of the reconstructed outside-target centroid, and seeded repeatability.
...
- **Reproduce:** [`tests/test_electropermanent_controller.py::test_equivalent_sphere_field_has_expected_axial_and_equatorial_signs`](https://github.com/supertjhok/MRSpinDynamics/blob/main/PythonSpinDynamics/tests/test_electropermanent_controller.py#L115); [`tests/test_electropermanent_controller.py::test_contrast_estimator_returns_resolved_foci_without_particle_truth`](https://github.com/supertjhok/MRSpinDynamics/blob/main/PythonSpinDynamics/tests/test_electropermanent_controller.py#L131); [`tests/test_electropermanent_controller.py::test_spin_echo_uses_epm_b0_and_particle_subvoxel_field`](https://github.com/supertjhok/MRSpinDynamics/blob/main/PythonSpinDynamics/tests/test_electropermanent_controller.py#L150); [`tests/test_electropermanent_controller.py::test_controller_can_use_susceptibility_spin_echo_feedback`](https://github.com/supertjhok/MRSpinDynamics/blob/main/PythonSpinDynamics/tests/test_electropermanent_controller.py#L289); [`examples/plot_epm_particle_spin_echo.py`](https://github.com/supertjhok/MRSpinDynamics/blob/main/PythonSpinDynamics/examples/plot_epm_particle_spin_echo.py)
- **Limitations:** The equivalent-sphere field does not yet model rod orientation, nonspherical demagnetization, particle-particle magnetic interaction, aggregation-dependent magnetization, or microscopic water exchange. The spin-warp engine models a two-dimensional slice with ideal rectangular RF drive, scalar B1 maps, Cartesian gradients, and a mesoscopic subvoxel/random-walker approximation; it is not a scanner-validated Bloch-Torrey calculation. Target contrast fraction is not particle occupancy or particle count. Static spin echo suppresses susceptibility sensitivity, while finite-bandwidth and readout effects cause blooming and can shift the recovered centroid. The example uses an illustrative 100 um effective magnetic core and disables receiver noise by default to isolate sequence physics; it is not an injectable-particle detectability claim.
...
- **Claim:** The controller executes contiguous image-program-transport-verification intervals, supports both the calibrated direct-signal baseline and susceptibility-aware paired spin echoes, re-aims each affine transport objective from the reconstructed outside-target centroid, and stops reproducibly from verification imaging rather than simulator truth.
- **Evidence:** A, R (partially validated)
...
- **Reproduce:** [`tests/test_electropermanent_controller.py::test_peak_relative_localization_returns_weighted_centroid`](https://github.com/supertjhok/MRSpinDynamics/blob/main/PythonSpinDynamics/tests/test_electropermanent_controller.py#L26); [`tests/test_electropermanent_controller.py::test_cloud_in_cell_image_preserves_count_and_centroid`](https://github.com/supertjhok/MRSpinDynamics/blob/main/PythonSpinDynamics/tests/test_electropermanent_controller.py#L47); [`tests/test_electropermanent_controller.py::test_image_estimator_recovers_count_positions_centroid_and_occupancy`](https://github.com/supertjhok/MRSpinDynamics/blob/main/PythonSpinDynamics/tests/test_electropermanent_controller.py#L64); [`tests/test_electropermanent_controller.py::test_boundary_capture_correction_uses_grid_geometry_not_particle_truth`](https://github.com/supertjhok/MRSpinDynamics/blob/main/PythonSpinDynamics/tests/test_electropermanent_controller.py#L84); [`tests/test_electropermanent_controller.py::test_noisy_particle_imaging_is_reproducible_and_image_resolved`](https://github.com/supertjhok/MRSpinDynamics/blob/main/PythonSpinDynamics/tests/test_electropermanent_controller.py#L251); [`tests/test_electropermanent_controller.py::test_controller_can_use_susceptibility_spin_echo_feedback`](https://github.com/supertjhok/MRSpinDynamics/blob/main/PythonSpinDynamics/tests/test_electropermanent_controller.py#L289); [`tests/test_electropermanent_controller.py::test_controller_alternates_modes_reaims_and_accumulates_capture`](https://github.com/supertjhok/MRSpinDynamics/blob/main/PythonSpinDynamics/tests/test_electropermanent_controller.py#L339); [`tests/test_electropermanent_controller.py::test_seeded_controller_is_reproducible`](https://github.com/supertjhok/MRSpinDynamics/blob/main/PythonSpinDynamics/tests/test_electropermanent_controller.py#L376); [`tests/test_electropermanent_transport.py::test_initial_capture_mask_keeps_particles_immobilized`](https://github.com/supertjhok/MRSpinDynamics/blob/main/PythonSpinDynamics/tests/test_electropermanent_transport.py#L159); [`examples/plot_epm_closed_loop_controller.py`](https://github.com/supertjhok/MRSpinDynamics/blob/main/PythonSpinDynamics/examples/plot_epm_closed_loop_controller.py)
- **Limitations:** The direct-signal baseline uses a supplied signal per aggregate and is not a physical MR contrast mechanism. The spin-echo option is physical but uncalibrated and its target contrast fraction is not a particle count or occupancy measurement. Recovered positions are representative image-resolved distribution samples or susceptibility foci, not identity-preserving tracks. The direct-channel boundary-capture partial-volume correction follows from the simulated first-entry capture rule and grid geometry; it requires experimental validation and does not apply to blooming spin-echo contrast. Summed retained-remanence variation is a programming-effort metric, not pulse count, switching energy, temperature, or driver stress. A calibrated 72-channel programming schedule is required for those quantities. The controller inherits the illustrative array, aggregate, dilute transport, irreversible circular capture, and missing vascular/tissue physics of the component models.
