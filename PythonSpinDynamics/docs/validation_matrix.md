...
| Alternating image-guided EPM therapy controller | The controller executes contiguous image-program-transport mode intervals, re-localizes a known target from independently seeded acquisitions, re-aims each affine transport objective from the uncaptured-particle centroid, preserves prior captures, and stops reproducibly at its capture goal or cycle limit. | **A**, **R** | partially validated |
...
- **Claim:** The controller executes contiguous image-program-transport mode intervals, re-localizes a known target from independently seeded acquisitions, re-aims each affine transport objective from the uncaptured-particle centroid, preserves prior captures, and stops reproducibly at its capture goal or cycle limit.
- **Evidence:** A, R (partially validated)
- **Basis:** State-machine interval identities, direct centroid-direction geometry, seeded nonlinear reconstructions and Brownian paths, retained-state total variation, and monotone persistent-capture regressions.
- **Tested range:** One through four cycles in compact regressions and examples; 10-by-10 through 14-by-14 EPM encoding/force grids; 16 through 80 illustrative magnetic aggregates; configurable imaging, programming, and transport windows; capture goals in (0, 1].
- **Metric:** Mode order and contiguity, localization centroid, requested direction, remanence variation, force, capture monotonicity, stopping reason, and seeded repeatability
- **Tolerance:** Floating-point interval and direction identities; exact seeded output equality; non-decreasing persistent capture and exact configured stopping semantics.
- **References:** Feedback state-machine and centroid steering identities; Package nonlinear EPM imaging and magnetophoretic transport validation records
- **Reproduce:** [`tests/test_electropermanent_controller.py::test_peak_relative_localization_returns_weighted_centroid`](https://github.com/supertjhok/MRSpinDynamics/blob/main/PythonSpinDynamics/tests/test_electropermanent_controller.py#L20); [`tests/test_electropermanent_controller.py::test_controller_alternates_modes_reaims_and_accumulates_capture`](https://github.com/supertjhok/MRSpinDynamics/blob/main/PythonSpinDynamics/tests/test_electropermanent_controller.py#L97); [`tests/test_electropermanent_controller.py::test_seeded_controller_is_reproducible`](https://github.com/supertjhok/MRSpinDynamics/blob/main/PythonSpinDynamics/tests/test_electropermanent_controller.py#L116); [`tests/test_electropermanent_transport.py::test_initial_capture_mask_keeps_particles_immobilized`](https://github.com/supertjhok/MRSpinDynamics/blob/main/PythonSpinDynamics/tests/test_electropermanent_transport.py#L159); [`examples/plot_epm_closed_loop_controller.py`](https://github.com/supertjhok/MRSpinDynamics/blob/main/PythonSpinDynamics/examples/plot_epm_closed_loop_controller.py)
- **Limitations:** The target image is synthetic and the controller reads particle positions and capture state directly from the simulator; it does not yet infer particle concentration or location from MR measurements. Summed retained-remanence variation is a programming-effort metric, not pulse count, switching energy, temperature, or driver stress. A calibrated 72-channel programming schedule is required for those quantities. The controller inherits the illustrative array, aggregate, dilute transport, irreversible circular capture, and missing vascular/tissue physics of the component models.
