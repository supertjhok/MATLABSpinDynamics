...
- 34 capability-level claims
- 14 validated
- 15 partially validated
- 5 regression-only
...
| Alternating image-guided EPM therapy controller | The controller executes contiguous image-program-transport-verification intervals, reconstructs a calibrated particle-sensitive concentration channel, estimates count, image-resolved positions, centroids, and target occupancy, re-aims each affine transport objective from the estimated outside-target centroid, and stops reproducibly from verification imaging rather than simulator truth. | **A**, **R** | partially validated |
...
- **Claim:** The controller executes contiguous image-program-transport-verification intervals, reconstructs a calibrated particle-sensitive concentration channel, estimates count, image-resolved positions, centroids, and target occupancy, re-aims each affine transport objective from the estimated outside-target centroid, and stops reproducibly from verification imaging rather than simulator truth.
- **Evidence:** A, R (partially validated)
...
- **Reproduce:** [`tests/test_electropermanent_controller.py::test_peak_relative_localization_returns_weighted_centroid`](https://github.com/supertjhok/MRSpinDynamics/blob/main/PythonSpinDynamics/tests/test_electropermanent_controller.py#L23); [`tests/test_electropermanent_controller.py::test_cloud_in_cell_image_preserves_count_and_centroid`](https://github.com/supertjhok/MRSpinDynamics/blob/main/PythonSpinDynamics/tests/test_electropermanent_controller.py#L44); [`tests/test_electropermanent_controller.py::test_image_estimator_recovers_count_positions_centroid_and_occupancy`](https://github.com/supertjhok/MRSpinDynamics/blob/main/PythonSpinDynamics/tests/test_electropermanent_controller.py#L61); [`tests/test_electropermanent_controller.py::test_boundary_capture_correction_uses_grid_geometry_not_particle_truth`](https://github.com/supertjhok/MRSpinDynamics/blob/main/PythonSpinDynamics/tests/test_electropermanent_controller.py#L81); [`tests/test_electropermanent_controller.py::test_noisy_particle_imaging_is_reproducible_and_image_resolved`](https://github.com/supertjhok/MRSpinDynamics/blob/main/PythonSpinDynamics/tests/test_electropermanent_controller.py#L159); [`tests/test_electropermanent_controller.py::test_controller_alternates_modes_reaims_and_accumulates_capture`](https://github.com/supertjhok/MRSpinDynamics/blob/main/PythonSpinDynamics/tests/test_electropermanent_controller.py#L197); [`tests/test_electropermanent_controller.py::test_seeded_controller_is_reproducible`](https://github.com/supertjhok/MRSpinDynamics/blob/main/PythonSpinDynamics/tests/test_electropermanent_controller.py#L234); [`tests/test_electropermanent_transport.py::test_initial_capture_mask_keeps_particles_immobilized`](https://github.com/supertjhok/MRSpinDynamics/blob/main/PythonSpinDynamics/tests/test_electropermanent_transport.py#L159); [`examples/plot_epm_closed_loop_controller.py`](https://github.com/supertjhok/MRSpinDynamics/blob/main/PythonSpinDynamics/examples/plot_epm_closed_loop_controller.py)
- **Limitations:** The target and particle images are synthetic separate channels. Particle signal per aggregate is a supplied calibration, and experimental contrast separation, acquisition mismatch, and detectability have not been validated. Recovered positions are representative image-resolved distribution samples, not identity-preserving tracks. The boundary-capture partial-volume correction follows from the simulated first-entry capture rule and grid geometry; it requires experimental validation and should be disabled for a different capture mechanism. Summed retained-remanence variation is a programming-effort metric, not pulse count, switching energy, temperature, or driver stress. A calibrated 72-channel programming schedule is required for those quantities. The controller inherits the illustrative array, aggregate, dilute transport, irreversible circular capture, and missing vascular/tissue physics of the component models.
