...
> **Status (July 2026):** The original V3 port priorities are complete. This
...
| `sim_spin_dynamics_arb` | `spin_dynamics.core.kernels` | `sim_spin_dynamics_arb10.m` is the principal parity reference. |
...
| `calc_echo` | `spin_dynamics.core.echo` | `calc_time_domain_echo.m` is the principal parity reference. |
...
| `FID_Example`, `Sim_FID` | `spin_dynamics.workflows.fid` | Ideal and probe-aware FID workflows are available. |
...
## Original Port Priorities (Complete)
...
| Matched diffusion CPMG | `sim_spin_dynamics_arb10_diffusion`, `calc_macq_matched_probe_relax_diffusion`, `run_matched_diffusion_cpmg`, and `run_matched_diffusion_q_sweep` provide the MATLAB-derived path; newer deterministic and walker diffusion workflows live alongside it. |
