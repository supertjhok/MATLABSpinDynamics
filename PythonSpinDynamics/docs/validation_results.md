...
| `spin_dynamics.workflows.acquire_pgse_qspace_walkers` | Finite-pulse restricted-diffusion extension of the ideal q-space inverse | seeded elliptical-pore walker grid, zero-q normalization, ideal-autocorrelation correlation above 0.9, preserved axis anisotropy, and support-constrained pore-mask IoU above 0.6 | Passed |
| q-space incomplete-coverage and finite-SNR reconstruction | Magnitude-only pore imaging beyond the ideal/full-grid limit | repeated ellipse, slit, and connected-domain retrievals; explicit measured-sample masks; radial q-window and random-dropout studies; shift/reflection-invariant metrics; and two-sigma noise-floor gating that restores IoU ≥ 0.5 in all default combined-condition trials | Passed |
...
| `spin_dynamics.optimization.validate_inverse_excitation_pair` | Analytical exact-cancellation and controlled amplitude/phase-error limits | integrated residual, peak residual, inverse coherence, and optional SNR-balance diagnostics | Passed |
