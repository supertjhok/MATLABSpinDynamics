...
| 5b ✅ | Measurement-level spectroscopy facade: full-model `NQRFID`, reduced spin-1 `NQRPopulationTransfer`, `ESRCWSweep`, and `ESRDEER`; DEER distance distributions serialize with the experiment and its array result is wrapped for normal NPZ provenance. | closes the primary NQR/ESR facade gaps; ESEEM/HYSCORE/ENDOR remain direct-call |
