...
- reciprocal receiver arrays with channel-resolved CPMG imaging, Roemer
  combination, Cartesian SENSE, passive/active coupled networks, resonant
  cancellation, LNA/interconnect studies, and the dedicated resonant,
  loaded/matched birdcage topology with reciprocal reception;
...
- receiver/RF hardware extensions beyond the current analytical and lumped
  layers: general arbitrary circuit graphs, measured S-parameter and standard
  LNA noise-parameter import, coupled transmission lines, component
  tolerance/loss propagation, full-wave dielectric/shield modeling, SAR,
  compression/recovery, and scanner or bench validation;
...
- continued distribution/API polish and broader measured validation. Typed
  package metadata, API lifecycle policy, generated API/evidence inventories,
  and the published documentation site are now in place and checked by CI.
